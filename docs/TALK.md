...
> **This document has a date on it.** Everything here is stage direction for one afternoon —
> timings, tab order, what to say while a spinner turns. The part that outlives the talk is in
> [RUNBOOK.md](./RUNBOOK.md): how to run the loop, and what to do when it misbehaves.
>
> It was one file until the two halves started rotting at different speeds. The failure table
> was describing a pipeline that had been split in two, and nobody noticed, because it lived
> under a heading about a presentation.
