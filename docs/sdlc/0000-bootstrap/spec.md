...
│       ├── agent-review.yml      # AGENT 3: PR -> /code-review --comment
...
└── site/                         # Astro
    ...
    └── tests/screenshots.spec.ts  # Playwright
```
...
| 4 Test | Every push | `verify.yml`: build, link check, screenshots, evals | — | **free** |
| 5 Deploy | PR opened, then merge | `agent-review.yml` comments; `preview.yml` posts a URL; `deploy.yml` waits on the gate | You approve the Environment | ~1 agent run |
...
| 3 | `agent-review.yml` | automation, on pull request | Review comments. Installed free by `/install-github-app`. |
...
`astro build` + `linkinator` over the output + Playwright screenshots of each section.
One command, three signals, no tokens.
...
(skill applied as a constraint → screenshot test → preview URL → gate), and the audience can
